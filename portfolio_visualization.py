...
def vis_portfolio_from_csv_subplots(
        companies: list[Company], prediction_time: int,
        weights: Iterable[float],
        csv_directory: str,
        plot_timeline_start: int = 0,
    ...
    for i, company in enumerate(companies):
        ...
        prediction_value = return_estimation.expected_return_from_csv(csv_path, prediction_time)
        ax[row, column].plot(company.prices)
        ax[row, column].set_xlim([plot_timeline_start, prediction_time])
        ax[row, column].plot(all_predictions[0], all_predictions[1])
        ax[row, column].set_title(
            f"{company.name}: " +
            f"{np.round((prediction_value/company.prices[-1]-1)*100, 2)}% " +
            f"{np.round(weights[i], 2)} | "
            f"{np.round((prediction_value/company.prices[-1]-1) * weights[i], 2)}")
        ax[row, column].tick_params(
            axis='x', which='both', bottom=False, top=False, labelbottom=False)
    # fig.tight_layout()
    plt.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05)
...
if __name__ == "__main__":
    ...
    ordered_weights = utils.weights_in_valid_order(companies, weights_portfolio1)
    for company, weight in zip(utils.COMPANY_ORDER, ordered_weights):
        print(f"{company:19} & {round(weight, 5):0f}")
